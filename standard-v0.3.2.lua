...
local Window = Library.CreateLib("Standard Script Roblox version 0.3.2 [Beta] By.carfty", "BloodTheme") -- ใช้ธีม BloodTheme

-- ✅ Standard Tab
local standard = Window:NewTab("Standard")

-- ✅ Aimbot & FOV Settings Tab
local aimbotFovSection = standard:NewSection("Aimbot and FOV Settings")
...
-- Aimbot Toggle
aimbotFovSection:NewToggle("Enable Aimbot", "เปิด/ปิด Aimbot", function(state)
...
aimbotFovSection:NewSlider("FOV Amount", "ปรับขนาด FOV", 120, 70, function(amount)
    FOVAmount = amount
    local camera = game.Workspace.CurrentCamera
    if FOVEnabled then
        camera.FieldOfView = FOVAmount -- ปรับ FOV ตามค่าใหม่
    end
end)

-- Aimbot ตัวเลือก Lock Part
aimbotFovSection:NewDropdown("Lock Body Part (Default Head)", "เลือกส่วนที่ต้องการล็อกเป้าหมาย (In the development process)", {"Head"}, function(part)
    Aimbot.LockPart = part
end)

aimbotFovSection:NewButton("Reset FOV", "รีเซ็ตFOV กลับค่าเริ่มต้น", function()
    AimbotEnabled = false
    FOVEnabled = true
    FOVAmount = 90
    local camera = game.Workspace.CurrentCamera
